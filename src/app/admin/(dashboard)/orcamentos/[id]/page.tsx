...
export default async function OrcamentoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      <Link href="/admin/orcamentos" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-800 mb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-5">
          <div className="bg-white border border-slate-200 rounded-xl p-6">
            ...
            <p className="text-slate-500 text-sm">{formatDateTime(q.created_at)}</p>
          </div>
          ...
          {q.description && (
            <div className="bg-white border border-slate-200 rounded-xl p-6">
              ...
              <p className="text-slate-500 text-sm leading-relaxed">{q.description}</p>
            </div>
          )}
        </div>
